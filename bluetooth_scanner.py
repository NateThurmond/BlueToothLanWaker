...
def start_dbus_bt_monitor(callback_fn) -> None:
    """
    Linux-only: poll BlueZ via D-Bus every second for paired device connections.

    When a controller paired with this Pi presses its button, BlueZ auto-accepts
    the directed advertisement and marks the device Connected=True.  We detect
    that here and fire the WoL callback — no pairing mode needed.

    One-time setup required:
        sudo bluetoothctl
        [bluetooth]# power on
        [bluetooth]# agent on
        [bluetooth]# scan on          # put controller in pairing mode first
        [bluetooth]# pair <MAC>
        [bluetooth]# trust <MAC>
        [bluetooth]# quit

    Requires: sudo apt install libdbus-1-dev && pip install dbus-python
    """
    ...
    try:
        ...
        logger.info(
            "dbus-python not installed — paired-device detection disabled. "
            "Run: sudo apt install libdbus-1-dev && pip install dbus-python"
        )
        return

    logger.info("BlueZ D-Bus connection monitor started")
    known_connected: set = set()
    import time as _t
    ...
    while True:
        try:
            ...
            for path, interfaces in objects.items():
                ...
                if addr:
                    current_connected.add(addr)
                    if addr not in known_connected:
                        logger.info("Paired device connected via BlueZ: %s (%s)", addr, name)
                        callback_fn({"mac": addr, "name": name, "device_type": None})
                        # Immediately disconnect so the controller falls back to
                        # connecting to the PS5/Xbox (which is now waking up).
                        try:
                            dev_obj = bus.get_object("org.bluez", path)
                            dev_iface = _dbus.Interface(dev_obj, "org.bluez.Device1")
                            dev_iface.Disconnect()
                            logger.info("Auto-disconnected %s — controller will reconnect to game console", addr)
                        except Exception as disc_err:
                            logger.debug("Auto-disconnect failed for %s: %s", addr, disc_err)

            known_connected = current_connected

        except Exception as exc:
            logger.debug("D-Bus poll error: %s", exc)
